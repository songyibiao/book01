...
有些好奇萌在这里会产生困惑：为什么中断服务函数里的代码写法这么奇怪？为什么要每 1ms 单独运行一部分代码？既然要 10ms 运行一次代码，那么我直接把中断时间改成 10ms，然后在 10ms 中断里运行全部代码不更简单吗？对，你的理解是对的。按你所想的来做也是正确的。但是，因为我们这里使用的是滴答定时器 SysTick，默认就是 1ms 运行一次，而且 HAL 库里面的延时函数 HAL_Delay() 也是以滴答定时器 SysTick 为基准获得延时的时间，如果我们把 SysTick 改成了 10ms，那么还需要改动 HAL 库中的相关函数代码，更加麻烦。
